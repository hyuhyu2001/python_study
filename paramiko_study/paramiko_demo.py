...
import paramiko_study

#用用户名和密码连接
ssh = paramiko_study.SSHClient() #实例化SSH
ssh.set_missing_host_key_policy(paramiko_study.AutoAddPolicy()) #第一次连接远程服务器，做一个签名，在SSH下生成key
...
key = paramiko_study.RSAKey.from_private_key_file(private_key_path) #拿着我的私钥

ssh = paramiko_study.SSHClient()
ssh.set_missing_host_key_policy(paramiko_study.AutoAddPolicy())
...
t = paramiko_study.Transport(('182.92.219.86', 22))
t.connect(username='wupeiqi',password='WOshiniba8')
sftp = paramiko_study.SFTPClient.from_transport(t)
